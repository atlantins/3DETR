...
class BatchNormDim1Swap(nn.BatchNorm1d):
    ...
    def forward(self, x):
        ...
        hw, n, c = x.shape
        x = x.permute(1, 2, 0)
        x = super(BatchNormDim1Swap, self).forward(x) # 调用了nn.BatachNorm1d中的forward方法来进行batchnorm
...
NORM_DICT = {
    ...
    "bn1d": nn.BatchNorm1d,
    "id": nn.Identity, # 输入是啥输出就是什么，是用来做标识的
    "ln": nn.LayerNorm,
}
...
ACTIVATION_DICT = {
    "relu": nn.ReLU,
    "gelu": nn.GELU,
    "leakyrelu": partial(nn.LeakyReLU, negative_slope=0.1),  # 当x<0的时候的梯度
}
