...
def shift_scale_points(pred_xyz, src_range, dst_range=None):
    ...
    if pred_xyz.ndim == 4:
        src_range = [x[:, None] for x in src_range] # 在b的后面添加一个新的维度
    ...
    src_diff = src_range[1][:, None, :] - src_range[0][:, None, :] # 假设是 max里面 B，N，3 -> B , 1, N, 3,计算src的最大值和最小值的差异
